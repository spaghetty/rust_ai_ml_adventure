...
fn test_module_creation() {
    ...
    assert_eq!(module.linear.bias.expect("Bias should exist").shape().dims, [1]);
}
...
fn test_module_training() {
    ...
    let initial_loss = (initial_output.clone() - target.clone()).powf_scalar(2.0).mean();
...
fn test_module_parameter_updates() {
    ...
    let initial_bias = module.linear.bias.clone().expect("Bias should exist").val().clone();
    ...
    assert!(module.linear.bias.expect("Bias should exist").val().clone().into_data() != initial_bias.clone().into_data());
}
...
fn test_module_inference() {
    ...
    assert!(!output.into_data().to_vec::<f32>().expect("Output should be a vector").contains(&f32::NAN));
}
...
fn test_module_configuration() {
    ...
    assert_eq!(module.linear.bias.expect("Bias should exist").shape().dims, [5]);
}
...
fn test_module_multiple_layers() {
    ...
    assert_eq!(module.linear.bias.expect("Bias should exist").shape().dims, [3]);
}
...
fn test_module_gradient_flow() {
    ...
    assert!(module.linear.bias.expect("Bias should exist").grad(&gradients).is_some());
}
